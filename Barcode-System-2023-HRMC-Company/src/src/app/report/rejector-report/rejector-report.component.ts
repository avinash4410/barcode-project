...
@Component({
  selector: 'app-rejector-report',
  templateUrl: './rejector-report.component.html',
  styleUrls: ['./rejector-report.component.scss']
})
export class RejectorReportComponent {
  data: any
  ...
  searchTerm: any;
  barcodeData: any;
  constructor(private service: ReportService, private exportSerrive: ExportService, private supervisorService: SupervisorService, public datepipe: DatePipe) { }
  ngOnInit(): void {
    this.getAll()
  ...
  printBarcodeInfo() {
    let pdf = new jsPDF();
    pdf.text('Rejector Timesheet', 11, 8);
    pdf.setFontSize(20)
    // let testcolumns = ["TestCol1", "TestCol2"];
    // let testrows = [["test item 1", "test item 2"]];
    let tableHeader = ['Job Card', 'Barcode', 'Scanned Date']
    let tableBody = this.barcodeData.map((d: any) => {
      let arr = ([d.cardNo, d.barcode, this.datepipe.transform(d.SCAN_DATE, 'dd MMM yyyy hh:mm:ss')])
      return arr;
    })
  ...
  exportPDF(): void {
    ...
    let pdf = new jsPDF();
    pdf.text('REJECTOR REPORT', 11, 8);
    pdf.setFontSize(20)
    // let testcolumns = ["TestCol1", "TestCol2"];
    // let testrows = [["test item 1", "test item 2"]];
    let tableHeader = ['JOB CARD', 'REJECTOR NAME','TASK WORK QUANTITY','START TIME','END TIME']
    let tableBody = this.data.map((d: any) => {
      let arr = ([d.cardNo, d.rejectorName, d.totalokQty,this.datepipe.transform(d.RejectorStartTime,'dd-mmm-yyy hh:mm:ss'), this.datepipe.transform(d.RejectorEndTime,'dd-mmm-yyy hh:mm:ss')])
      return arr;
    })
    console.log("PDF Data", tableBody);
    (pdf as any).autoTable(tableHeader, tableBody);
  ...
  modalData(item: any) {
    this.service.getBarcodeByCardNo(item.cardNo).subscribe((res: any) => {
      ...
      let allBarcodes = res;
      let a = allBarcodes.filter((barData: any) => {
        if (barData.status == 'rejectorVerified')  /* && barData.SCAN_DATE!=null */
          return barData
      })
      this.barcodeData = a
    })
